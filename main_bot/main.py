...
@dp.callback_query(lambda call: call.data == 'apply')
async def application_start(call: types.CallbackQuery, state: application_state.Application.first_question):
    if call.data == 'apply':
...
@dp.message(StateFilter(application_state.Application.first_question))
async def application_fist(message: Message, state: application_state.Application.first_question):
    answer = message.text
...
@dp.message(StateFilter(application_state.Application.second_question))
async def application_second(message: Message, state: application_state.Application.first_question):
    answer = message.text
...
@dp.message(StateFilter(application_state.Application.third_question))
async def application_third(message: Message, state: application_state.Application.third_question):
    answer = message.text
    state_info = await state.get_data()
    first_answer = state_info.get('first_question')
...
@dp.callback_query(lambda call: call.data in ['send_application', 'again'])
async def application_send(call: types.CallbackQuery, state: application_state.Application.first_question):
...
@dp.callback_query(lambda call: call.data in ['accept', 'deny'])
